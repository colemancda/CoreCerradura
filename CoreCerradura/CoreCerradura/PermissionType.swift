...
public enum PermissionType: String {
    ...
    /* Permanent permission. Permission granted by owner / admin indefinitely. User's with permanent permission cannot grant permission to other users. */
    public case Permanent = "permanent"
    
    /* Guest permission. Permission granted by owner / admin for a specified amount of time. User's with guest permission for a lock cannot create permissions for other users. */
